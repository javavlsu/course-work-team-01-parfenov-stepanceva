      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div className="bg-paper border border-gray-100 rounded-lg p-6">
          ...
          <form onSubmit={sendEmail} className="space-y-3">
            ...
            <Button type="submit" fullWidth loading={inviteEmail.isPending}>Отправить приглашение</Button>
          </form>
        </div>

        <div className="bg-paper border border-gray-100 rounded-lg p-6">
          <div className="mono text-xs tracking-[0.1em] uppercase text-gray-400 mb-4">По ссылке</div>
          <p className="text-sm text-gray-600 mb-4">Универсальная ссылка. Кто её откроет (и авторизован) — вступит в группу.</p>
          <Button onClick={genLink} variant="outline" fullWidth icon={Link2} loading={inviteLink.isPending}>Создать ссылку</Button>
        </div>
      </div>

      <div>
        <div className="mono text-xs tracking-[0.1em] uppercase text-gray-400 mb-3">Активные приглашения</div>
        {isLoading ? (
          <div className="skeleton h-24 rounded-lg" />
        ) : invites.length === 0 ? (
          <div className="border border-dashed border-gray-200 rounded-lg p-8 text-center text-sm text-gray-500">Нет активных приглашений</div>
        ) : (
                ...
                <div key={inv.id} className="flex items-center gap-4 px-4 py-3 border-t first:border-t-0 border-gray-100">
                  ...
                  <div className="flex-1 min-w-0 text-sm">
                    ...
                    <div className="text-xs text-gray-400">
                      {inv.status} · до {formatDate(inv.expiresAt)}
                    </div>
                  </div>
                  {fullLink && (
                    <button
                  ...
                  <button
                    onClick={() => { if (confirm('Отозвать приглашение?')) cancelInvite.mutate(inv.id) }}
